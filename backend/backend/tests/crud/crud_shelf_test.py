...
def test_create(db) -> None:
    ...
    # Assert
    assert first_shelf.id == 2  # 2 since there already exist a base shelf
...
def test_create_with_owner(db) -> None:
    ...
    # Assert
    assert test_shelf.id == 3  # 3 since there already exist a base shelf, and previous created shelf
